...
$id_setor = isset($_GET['id_setor']) ? intval($_GET['id_setor']) : 02;
$id_projeto = isset($_GET['id_projeto']) ? intval($_GET['id_projeto']) : 01;
$caminho_projeto = isset($_GET['caminho_projeto']) ? urldecode($_GET['caminho_projeto']) : '';
//echo"<h1> $id_setor</h1>";
//echo"<h1> $id_projeto</h1>";
/*
...
$chave_sql ="SELECT * FROM relatorios WHERE id_setor='$id_setor'";
        ...
        if(isset($_POST["add"])){
            if(isset($_POST["busca"])){
                if (!empty($_POST['mostrar'])) {
                    ...
                    $chave_verifica= "SELECT * FROM col_projeto WHERE login_colaborador='$login_do_colaborador' AND id_setor='$id_setor'";
                    $verificacao_result = $mysqli->query($chave_verifica);
                    if ($verificacao_result->num_rows > 0) {
                        $text = "Esté funcionario já está nesse setor";
                        echo"<h3>$text</h3>";
                    }else{
                        $data =  date("Y-m-d H:i:s");
                    $chave_sql_adicionar = "INSERT INTO col_projeto(login_colaborador, id_setor, id_projeto, dt_entrada)VALUES('$login_do_colaborador', '$id_setor','$id_projeto', '$data')";
